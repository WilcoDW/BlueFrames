...
newHook("cap",function()
 ...
 if coroutine.status(p[q].co)=="dead" then
  if focus==q then
		 shared.needupdate=true
   focus=1
  end
...
newHook("sap",function()
 if isFaked and shared.render and (shared.forceupdate or shared.needupdate) then
  ...
  u.term.setCursorPos(1,17)
  u.term.write("_"..string.rep("_",realw-1))
  u.term.setCursorPos(1,18)
  u.term.write("|")
  local ofs=0
  for a,b in ipairs(p) do
   if a==1 then
    ...
    if b.living then
     if a==focus then
      u.term.setCursorPos((ofs*shared.tasksize)+3,17)
      u.term.write("+"..string.rep("-",shared.tasksize-1).."+")
     end
     u.term.setCursorPos((ofs*shared.tasksize)+4,18)
     u.term.write(b.title:sub(1,(shared.tasksize-1)))
     u.term.setCursorPos(((ofs+1)*shared.tasksize)+3,18)
     u.term.write("|")
     ofs=ofs+1
    end
   end
...
-- end of mod installation, now starting:
